-- RPC: recommend_cars
-- Filters variants by price, optional body types (family → SUV/MPV),
-- scores by usage + priority, groups by model, returns top results ranked.
  ...
  with scored as (
    select
      ...
      (
        ...
        -- priority: reviews (NCAP as proxy until reviews exist)
    ...
    where v.ex_showroom_price between p_min_price and p_max_price
      and (
        -- if family in usage, restrict to SUV/MPV
        not ('family' = any(p_usage))
        or m.body_type in ('SUV', 'MPV')
      )
  ),
  -- rank variants within each model by score desc, pick the best
  ranked as (
    select *,
